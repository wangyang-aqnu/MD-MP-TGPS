...
def RC_item_to_user(R_item_to_user,SD_item_to_user,O1):
...
def RC_tag_to_item(R_tag_to_item,SD_tag_to_item,ST2_R_tag):
...
def iteration(n,R_item_to_user,SD_item_to_user,R_user_to_tag,SD_user_to_tag,R_tag_to_item,SD_tag_to_item,O1):
    iter_n=0
    ST1_R_user=RC_item_to_user(R_item_to_user,SD_item_to_user,O1)
    ST2_R_tag=RC_user_to_tag(R_user_to_tag,SD_user_to_tag,ST1_R_user)
    ST3_R_item=RC_tag_to_item(R_tag_to_item,SD_tag_to_item,ST2_R_tag)
    while (iter_n <n):
        ST1_R_user=RC_item_to_user(R_item_to_user,SD_item_to_user,ST3_R_item)
        ST2_R_tag=RC_user_to_tag(R_user_to_tag,SD_user_to_tag,ST1_R_user)
        ST3_R_item=RC_tag_to_item(R_tag_to_item,SD_tag_to_item,ST2_R_tag)
        iter_n+=1
    return ST3_R_item
...
def sorted_R_item(ST3_R_item):
    sorted_R_item_list=sorted(ST3_R_item.items(),key=lambda item:item[1],reverse=True)
...
####################################
    ...
    ST3_R_item=iteration(0,R_item_to_user,SD_item_to_user,R_user_to_tag,SD_user_to_tag,R_tag_to_item,SD_tag_to_item,O1)
...
#####################################
    ...
    sorted_R_item_list=sorted_R_item(ST3_R_item)
    #print("推荐列表:\n",sorted_R_item_list)
...
f=open('3D-IUAI_1.txt','a')
f.write("u1:\n")
for i in range(10,110,10):
#for de in np.arange(0,1.01,0.01):
  avg_HR,avg_ILD,Ham_dist,avg_PO,Apris=pers_recom("train1.dat","test1.dat",i,15,1)
