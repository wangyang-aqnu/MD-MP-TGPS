...
#########################item:{user:score}
      if itemid not in prefs_item_user:
          ...
          if userid not in prefs_item_user[itemid]:
              prefs_item_user[itemid][userid]=1.0*math.exp(0.0577623*int((float(ts)-tmax)/(86400*eta)))
          else:
              prefs_item_user[itemid][userid]+=1.0*math.exp(0.0577623*int((float(ts)-tmax)/(86400*eta)))
      else:
           if userid not in prefs_item_user[itemid]:
...
#########################item:{tag:score}
      if itemid not in prefs_item_tag:
          ...
          if tagid not in prefs_item_tag[itemid]:
              prefs_item_tag[itemid][tagid]=1.0*math.exp(0.0577623*int((float(ts)-tmax)/(86400*eta)))
          else:
              prefs_item_tag[itemid][tagid]+=1.0*math.exp(0.0577623*int((float(ts)-tmax)/(86400*eta)))
      else:
           if tagid not in prefs_item_tag[itemid]:
...
######################### user:{item:score}
      if userid not in prefs_user_item:
          ...
          if itemid not in prefs_user_item[userid]:
              prefs_user_item[userid][itemid]=1.0*math.exp(0.0577623*int((float(ts)-tmax)/(86400*eta)))
          else:
              prefs_user_item[userid][itemid]+=1.0*math.exp(0.0577623*int((float(ts)-tmax)/(86400*eta)))
      else:
           if itemid not in prefs_user_item[userid]:
...
#########################user:{tag:score} 
      if userid not in prefs_user_tag:
          ...
          if tagid not in prefs_user_tag[userid]:
              prefs_user_tag[userid][tagid]=1.0*math.exp(0.0577623*int((float(ts)-tmax)/(86400*eta)))
          else:
              prefs_user_tag[userid][tagid]+=1.0*math.exp(0.0577623*int((float(ts)-tmax)/(86400*eta)))
      else:
           if tagid not in prefs_user_tag[userid]:
...
#########################user:{item:score}  tag:{item:score}
      if tagid not in prefs_tag_item:
          ...
          if itemid not in prefs_tag_item[tagid]:
              prefs_tag_item[tagid][itemid]=1.0*math.exp(0.0577623*int((float(ts)-tmax)/(86400*eta)))
          else:
              prefs_tag_item[tagid][itemid]+=1.0*math.exp(0.0577623*int((float(ts)-tmax)/(86400*eta)))
      else:
           if itemid not in prefs_tag_item[tagid]:
...
#########################tag:{item:score} tag:{user:score}
      if tagid not in prefs_tag_user:
          ...
          if userid not in prefs_tag_user[tagid]:
              prefs_tag_user[tagid][userid]=1.0*math.exp(0.0577623*int((float(ts)-tmax)/(86400*eta)))
          else:
              prefs_tag_user[tagid][userid]+=1.0*math.exp(0.0577623*int((float(ts)-tmax)/(86400*eta)))
      else:
           if userid not in prefs_tag_user[tagid]:
