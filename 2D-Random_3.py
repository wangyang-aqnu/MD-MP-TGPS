...
#对指定用户产生TOP L推荐--给定用户
def list_recs(list_unrating_items,L):
  list_recom=[]
  list_recom=sample(list_unrating_items,L)
  return list_recom
...
def com_PS(user,list_recom,prefs_user_test,list_unrating_items):
  ...
  for item in list_recom:
      if item in prefs_user_test[user]:
          p+=1
          position=list_recom.index(item)+1
...
####################################
    ...
    ST2_R_item=iteration(0,R_item_user,SD_item_user,R_user_item,SD_user_item,O1)
...
#####################################
    ...
    #对user用户产生TOP-L推荐
    list_recom=list_recs(list_unrating_items,L)
    #print("TOP-L推荐列表:\n",list_recom)
     ############################################
    '''
    #调用tr_list_recom
    ...
    list_recom_items_user=recom_list_items_user(list_recom,list_item_alluser)
    #print(list_recom_items_user)
    ############################################
    for list_recom1 in list_recom:
        list_all_recom[user].append(list_recom1) #把user用户推荐列表加入所有用户推荐列表中
    #####################################
    #user-lever的ILD计算
...
for i in range(10,110,10):
  ...
  for it_n in range(1,51,1):
    ...
    ILD1+=avg_ILD
    f.write("\n")
  f.write("i="+str(i)+'\t'+str(P/50)+'\t'+str(R/50)+'\t'+str(H/50)+'\t'+str(PO/50)+'\t'+str(ILD1/50))
  f.write("\n") 
  f.flush()
f.close()
