...
dic_pre_double,dic_pre_triple,dic_pre_hwL,dic_pre_hwA,dic_cumsum_items,dic_sum2_degree=ki.deg_K_items(r'./ml/35T/sam3/')
...
def com_populary(user,list_recom,prefs_user_test,L):
  ...
  for item,rating in list_recom:
      if item in dic_cumsum_items:
          rec_PO+=dic_cumsum_items[item]['21']      #######注意此处要随sam修改 
  #print(user,HR,"\\n")
  return rec_PO
...
f=open('3D-IUI-IAI_3.txt','a')
f.write("u3:\n")
for i in range(10,110,10):
#for de in np.arange(0,1.01,0.01):
  avg_HR,avg_ILD,Ham_dist,avg_PO,Apris=pers_recom("train3.dat","test3.dat",i,15,1,0.5)
