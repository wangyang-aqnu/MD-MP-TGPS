...
def TP_item():
    ST2_R_item={}
    for item in dic_sum2_degree:
        ST2_R_item.setdefault(item,0)
        ST2_R_item[item]=dic_cumsum_items[item]['21'] #总流行度#########################
        #ST2_R_item[item]=(x*(dic_sum2_degree[item]['23']+pow(10,-9)*dic_cumsum_items[item]['23'])+(1-x)*dic_cumsum_items[item]['23'])#混合流行度
        #ST2_R_item[item]=(dic_sum2_degree[item]['23'])+pow(10,-9)*dic_cumsum_items[item]['23'] #近来流行度
    #print(ST2_R_item)
    return ST2_R_item 
############################################    
