...
dic_pre_double,dic_pre_triple,dic_pre_hwL,dic_pre_hwA,dic_cumsum_items,dic_sum2_degree=ki.deg_K_items(r'./ml/35T/sam5/')
...
#############################################   
#项目到用户的评分权重字典R_item_to_user，引入时间衰减因子beta越近的评分权重越高
def prefs_item_tag_t(prefs_item_tag,delta):
    R_item_tag={}#带时间的项目对用户的评分权重
    SD_item_tag={}
    SD1_item_tag={}
    for item in prefs_item_tag:
        R_item_tag.setdefault(item,{})
        SD1_item_tag.setdefault(item)
        SD1_item_tag[item]=0
        for tag in prefs_item_tag[item]:
          R_item_tag[item][tag]=prefs_item_tag[item][tag]
          SD1_item_tag[item]+=R_item_tag[item][tag]    
          
    for item in SD1_item_tag:
       SD_item_tag.setdefault(item,0)
       SD_item_tag[item]=pow(SD1_item_tag[item],delta) 

    
    for item in R_item_tag:
        for tag in R_item_tag[item]:
            R_item_tag[item][tag]=R_item_tag[item][tag]*SD_item_tag[item]/SD1_item_tag[item] 
    return R_item_tag,SD_item_tag
    ...
    if tag in R_tag_item:
      for item in R_tag_item[tag]:
        if item not in ST3_R_item:
          ...
          ST3_R_item[item]=ST1_R_tag[tag]*R_tag_item[tag][item]/SD_tag_item[tag]
        else:
          ST3_R_item[item]+=ST1_R_tag[tag]*R_tag_item[tag][item]/SD_tag_item[tag]
  return ST3_R_item
#################################

#fusion of user-item and tag_item
def Fus_item(ST2_R_item,ST3_R_item,x):
  Fus_ST2_R_item={}
  
  for item in ST2_R_item:
    Fus_ST2_R_item.setdefault(item)
    if item in ST3_R_item:
      Fus_ST2_R_item[item]=(1-x)*ST2_R_item[item]+x*ST3_R_item[item]
    else:
      Fus_ST2_R_item[item]=(1-x)*ST2_R_item[item]

  
  for item in ST3_R_item:
    if item not in ST2_R_item:
      Fus_ST2_R_item.setdefault(item)
      Fus_ST2_R_item[item]=x*ST3_R_item[item]
  return Fus_ST2_R_item

####################################


#对指定用户按两步配置后项目资源量排序，产生排序项目列表
def sorted_R_item(Fus_ST2_R_item):
    sorted_R_item_list=sorted(Fus_ST2_R_item.items(),key=lambda item:item[1],reverse=True)
...
def com_populary(user,list_recom,prefs_user_test,L):
  ...
  for item,rating in list_recom:
      if item in dic_cumsum_items:
          rec_PO+=dic_cumsum_items[item]['23']      #######注意此处要随sam修改 
  #print(user,HR,"\\n")
  return rec_PO
...
###############################
#主程序  
def pers_recom(trainsetname,testsetname,L,tw,delta,x):
...
####################################
    ...
    #print("资源配置第一步由项目－用户传递后的用户资源量\n",ST1_R_user)
    
    ST2_R_item=RC_user_to_item(R_user_item,SD_user_item,ST1_R_user)
    #print("资源配置第二步由用户传回项目的项目资源量\n",ST2_R_item)
##################################### 
    
    ST2_R_item_tag=RC_tag_to_item(R_tag_item,SD_tag_item,O2)
#####################################
    Fus_ST2_R_item=Fus_item(ST2_R_item,ST2_R_item_tag,x)
#####################################
    #融合由用户和标签传回项目的项目资源量进行降序排列
    sorted_R_item_list=sorted_R_item(Fus_ST2_R_item)
    #print("推荐列表:\n",sorted_R_item_list)
...
f=open('3D-IUI-IAI_5.txt','a')
f.write("u5:\n")
for i in range(10,110,10):
#for de in np.arange(0,1.01,0.01):
  avg_HR,avg_ILD,Ham_dist,avg_PO,Apris=pers_recom("train5.dat","test5.dat",i,15,1,0.5)
